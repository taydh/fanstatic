...
Principles:
- QPanel is a front end developer point of view
...
var QPanel = function()
{
	...
	/* this.rootPanel */ this.__defineGetter__('rootPanel', function(){ return my.rootPanel; });
	...
	{		
		...
		var panel = new QPanel.Panel(me, container, id, templateName, templatePath);
	...
	//this.cleanPanels = function()
	//{
	//	var count = 0;
	//	
	//	for(var i=0; i<my.reg_panel.length; i++){
	//		if(QPanel.GetElement(my.reg_panel[i].id) == null){
	//			my.reg_panel[i].close();
	//			
	//			count++;
	//		}
	//	}
	//	
	//	console.log('!QPanel: ' + count + ' panel(s) cleaned');
	//}
...
{	
	...
	*/
			...
			if(!me.isControllerOff(templateName) && !my.disabledControllers){
				...
				my.qpanel.loadScript(me, templateName, function(isFailed){
					...
					if(!isFailed){
						...
						newController = QPanel.InjectController(my.qpanel, null, newController)
	...
	{			
		...
		try{
			console.log('QPanel: TemplatePath.evalScript: evaluating ' + templateName)
			eval(sEval);
		} catch(e){
			console.log('QPanel: Error evaluating script: ' + e.message)
		}
...
QPanel.Panel = function(qpanel, container, panelID, templateName, templatePath)
{
	/*	========
		Privates
		======== */
	...
	this.__defineGetter__("templateName", function(){return my.templateName;});
	this.__defineGetter__("templatePath", function(){return my.templatePath;});
	...
	this.close = function()
	{
		me.selfContainer.removePanel(me);
	}
	...
	this.getContainer = function(id)
	{
		...
		if(my.containers[id] == null){
			...
			if(div == null) throw('QPanel - Try to do getContainer() at panel "'+my.id+'" but no container element matching "'+id+'"') 
...
QPanel.Container = function(qpanel, containerID)
{
	...
	this.removePanel = function(panel)
	{
		...
		my.panels = QPanel.arrayRemoveByValue(panel, my.panels); 
		
		// 2/3 remove from QPanel registration
		my.qpanel.unregPanel(panel); 
	...
	this.moveToFront = function(panel)
	{
		my.panels = QPanel.ArrayRemoveByValue(panel, my.panels);
		my.panels.push(panel);
	...
	this.getPanelByTemplate = function(templatePath, templateName)
	{
		var panels = me.getPanelList();
		var res = null;
		
		for(var i=0; i<panels.length; i++)
		{
			if(panels[i].templatePath == templatePath && panels[i].templateName == templateName){
				res = panels[i];
				break;
			}
		}
